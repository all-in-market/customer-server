...
- 설치된 `terraform.exe`을 원하는 폴더에 위치
...
- 4단계에서 terraform apply 실행 후 출력된 아래 세개의 output 값을 
  - `infra/backend.tf` 에 매핑되는 이름의 설정 값 수정
  - `infra-grafana/backend.tf` 에 매핑되는 이름의 설정 값 수정
  - `infra-grafana/terraform.tfvars` 에 매핑되는 이름의 변수 값 수정
...
- `Amazon Q Developer in chat applications 콘솔`에서 `클라이언트 구성` 클릭
...
- `AWS IAM Identity Center 콘솔`에 들어가서 Region 선택 후, 활성화 버튼 클릭
- 인프라 생성 후 12 단계에서 이어서 진행
...
---
## 10. Grafana 대시보드 생성
- Grafana 내부 Resources 생성을 위해서는 Grafana Provider 가 필요
- Grafana Provider 초기화를 위해서는 아래 값들이 필요
...
- 인프라가 생성되면 `IAM Identity Center 콘솔`로 이동
...
- 다시 `IAM Identity Center 콘솔`로 이동하여 새로고침 후 `암호 재설정` 버튼을 클릭
- 발송된 메일을 통해 비밀번호 설정
...
- 9 단계에서 apply 후 출력된 output을 확인하여 GitHub Repository에서 아래와 같이 `GitHub Secrets`으로 등록
