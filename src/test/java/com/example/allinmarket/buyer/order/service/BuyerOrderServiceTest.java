...
class BuyerOrderServiceTest {
    ...
    class CreateOrderTest {
        ...
        void createOrder_success() {
            ...
            given(product1.getSeller()).willReturn(seller1);
            given(product2.getSeller()).willReturn(seller2);
            ...
            // when
            OrderDetailResponse result = buyerOrderService.createOrder(buyerId, cartItems, addressId);

            // then
            assertThat(result).isNotNull();

            verify(buyerRepository).findById(buyerId);
            verify(addressRepository).findByIdAndBuyerId(addressId, buyerId);

            verify(orderValidator).validateCartItemsNotEmpty(cartItems);
