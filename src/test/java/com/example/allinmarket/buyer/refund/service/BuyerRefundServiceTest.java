...
class BuyerRefundServiceTest {
    ...
    class GetRefundsTest {

        @Test
        @DisplayName("구매자의 환불 목록을 페이지로 반환한다")
        void getRefunds_success() {
            // given
            Long buyerId = 1L;
            Pageable pageable = PageRequest.of(0, 10);

            Buyer buyer = createBuyer(buyerId);
            Order order = createOrder(10L, buyer, new BigDecimal("15000"));
            Payment payment = createPayment(order, "payment_10_abc", new BigDecimal("15000"), MethodEnum.MOCK);
            setField(payment, "id", 100L);

            Refund refund = Refund.of(buyer, payment, ReasonEnum.CHANGE_OF_MIND, "단순 변심");
            setField(refund, "id", 1000L);

            given(refundRepository.findAllByBuyerId(buyerId, pageable))
                    .willReturn(new PageImpl<>(List.of(refund), pageable, 1));

            // when
            PageResponse<RefundDetailResponse> response = buyerRefundService.getRefunds(buyerId, pageable);

            // then
            assertThat(response.content()).hasSize(1);

            RefundDetailResponse content = response.content().get(0);
            assertThat(content.refundId()).isEqualTo(1000L);
            assertThat(content.buyerId()).isEqualTo(buyerId);
            assertThat(content.paymentId()).isEqualTo(100L);
            assertThat(content.reason()).isEqualTo(ReasonEnum.CHANGE_OF_MIND);
            assertThat(content.status()).isEqualTo(RefundStatus.PENDING);
        ...
        @Test
        @DisplayName("환불 내역이 없으면 빈 페이지를 반환한다")
        void getRefunds_empty() {
            // given
            Long buyerId = 1L;
            Pageable pageable = PageRequest.of(0, 10);
    ...
    @Nested
    @DisplayName("환불 단건 조회")
    class GetRefundTest {

        @Test
        @DisplayName("환불 단건 조회 성공")
        void getRefund_success() {
            ...
            Refund refund = Refund.of(buyer, payment, ReasonEnum.DAMAGED, "파손");
            setField(refund, "id", refundId);

            given(refundRepository.findByIdAndBuyerId(refundId, currentUserId))
                    .willReturn(Optional.of(refund));

            // when
            RefundDetailResponse response = buyerRefundService.getRefund(currentUserId, refundId);

            // then
            assertThat(response.refundId()).isEqualTo(refundId);
            assertThat(response.buyerId()).isEqualTo(currentUserId);
            assertThat(response.paymentId()).isEqualTo(100L);
            assertThat(response.reason()).isEqualTo(ReasonEnum.DAMAGED);
            assertThat(response.status()).isEqualTo(RefundStatus.PENDING);
        }

        @Test
        @DisplayName("환불이 존재하지 않으면 예외 발생")
        void getRefund_fail_notFound() {
            // given
            Long currentUserId = 1L;
            Long refundId = 1000L;
            ...
            BaseException ex = assertThrows(
                    BaseException.class,
                    () -> buyerRefundService.getRefund(currentUserId, refundId)
            );

            // then
            assertThat(ex.getErrorEnum()).isEqualTo(ErrorEnum.REFUND_NOT_FOUND);
        }

        @Test
        @DisplayName("다른 구매자의 환불은 조회되지 않는다")
        void getRefund_fail_otherBuyer() {
            // given
            Long currentUserId = 1L;
            Long refundId = 1000L;
            ...
            // then
            assertThat(ex.getErrorEnum()).isEqualTo(ErrorEnum.REFUND_NOT_FOUND);
            verify(refundRepository).findByIdAndBuyerId(refundId, currentUserId);
        }
    }
}
